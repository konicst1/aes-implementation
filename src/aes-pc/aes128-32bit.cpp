...
void initTBoxes() {
    uint8_t i = 0x00;  //this must be uint8_t in order to end loop correctly
    do {
        uint8_t sub_i = SBOX[i];
        uint8_t times_01 = sub_i;
        uint8_t times_02 = xtime(sub_i);
        uint8_t times_03 = xtime(sub_i) ^sub_i;
        T0[i] = word(times_02, times_01, times_01, times_03);
        T1[i] = word(times_03, times_02, times_01, times_01);
        T2[i] = word(times_01, times_03, times_02, times_01);
        T3[i] = word(times_01, times_01, times_03, times_02);
        i++;
    } while (i);
}

uint32_t tBoxTransformWord(uint8_t a0, uint8_t a1, uint8_t a2, uint8_t a3) {
    return (T0[a0] ^ T1[a1] ^ T2[a2] ^ T3[a3]);
}

void tBoxTransformState(t_state state) {
    uint32_t new_state_words[4];
    for (int i = 0; i < 4; i++) {
        new_state_words[i] = tBoxTransformWord(wbyte(state[i], 0), wbyte(state[(i + 1) % 4], 1), wbyte(state[(i + 2) % 4], 2), wbyte(state[(i + 3) % 4], 3));
    }
    for (int i = 0; i < 4; i++) {
        state[i] = new_state_words[i];
    }
}
...
int main(int argc, char *argv[]) {
    ...
    printf("Running AES128-32bit %d times.\n", rounds);
